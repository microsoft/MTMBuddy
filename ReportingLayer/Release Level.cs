...
namespace ReportingLayer
{
   ...
   //     public string Priority { get; set; }
       ...
       public float NewAutoRatio { get; set; }
        ...
        public static List<ReleaseLevel> Generate(string iteration,int planid,Dictionary<string, int> suitelist, string module = "", bool moduleinclusion = true, string tester = "", bool testerinclusion = true, string automationstatus = "both")
        {
            //rawData = rawData.OrderBy(l => l.Priority).ThenBy(l => l.Outcome).ToList();


            var reportList = new List<ReleaseLevel>();
            var rd = new List<resultsummary>();
            var rd1 = new List<resultsummary>();
   Parallel.ForEach(suitelist, suite =>
  // foreach (KeyValuePair<string, int> suite in suitelist)
            {
                var resDetail = new ConcurrentBag<resultsummary>();
                MTMInteraction.getsuiteresults(suite.Value, resDetail, suite.Key);

                var rawData = resDetail.ToList();
                var filtereddata = Utilities.filterdata(rawData, module, moduleinclusion, tester, testerinclusion,
                    automationstatus);

                ReleaseLevel modrep = new ReleaseLevel();
                modrep.Iteration = iteration;
                modrep.planid = planid;
                modrep.Type = "Application";
                modrep.SuiteName = suite.Key;
                ...
                if (modrep.Total > 0)
                {
                    modrep.AutomationRatio =
                        (float)
                            Math.Round(
                                (float) modrep.AutomationTotal/(modrep.AutomationTotal + modrep.ManualTotal)*100, 2);
                    ...
                    if (rd1.Count > 0)
                        modrep.NewAutoRatio =
                            (float)
                                Math.Round((float) modrep.AutomationNew/(modrep.AutomationNew + modrep.ManualNew)*100, 2);
                }
        ...
        public static List<ReleaseLevel> ProgramLevelGenerate(int planId,List<string> Programs,string iteration,List<string> AppList  , string module = "", bool moduleinclusion = true, string tester = "", bool testerinclusion = true, string automationstatus = "both")
        {
            //rawData = rawData.OrderBy(l => l.Priority).ThenBy(l => l.Outcome).ToList();
    ...
    //         foreach (string program in Programs)
            {
                var resDetail = new ConcurrentBag<resultsummary>();
                var eligiblesuitelist = "-1'";

                foreach (var suite in MTMInteraction.suitedictionary)
                {
